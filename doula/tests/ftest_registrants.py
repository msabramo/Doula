...
class Hub(object):
    ...
    def server_bind(self, socket):
        socket.bind_to_random_port(self.address)
        return self.address
...
def make_server(host, port, app, graceful=True, timeout=0.5, **kw):
    server = wsgi.WSGIServer((host, int(port)), app, **kw)                
    if graceful is True:
        gevent.signal(signal.SIGHUP, gserver.graceful_stop, server=server, huptimeout=timeout)
    return server
