...
from doula.models.commands.precondition_commands import *
...
class TestCommands(unittest.TestCase):
    ...
    def test_package_present(self):
        command = self.get_one(PythonPackagePresent)
        result = command.run('http://yorick:9003/index/requests-0.12.1.tar.gz')
        self.assertTrue(result)

        command = self.get_one(PythonPackagePresent)
        result = command.run('http://yorick:9003/index/requests-0.12.1.tar.gzzzzzzz')
        self.assertFalse(result)
